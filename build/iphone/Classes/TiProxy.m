...
//Rare exceptions to indicate a bug in the _kulturlotse code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_kulturlotse = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_kulturlotse]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.kulturlotse.module"
